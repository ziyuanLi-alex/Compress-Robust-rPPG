...
class JointSTVENPhysFormerTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        self.model = PhysFormerWithSTVEN(
            stven_config=config.MODEL.STVEN, physformer_config=config.MODEL.PHYSFORMER
        ).to(self.device)
    ...
    def _load_pretrained_weights(self, config):
        ...
        if config.MODEL.STVEN.PRETRAINED_PATH:
            ...
            stven_state = torch.load(
                config.MODEL.STVEN.PRETRAINED_PATH, map_location=self.device
            )
        ...
        if config.MODEL.PHYSFORMER.PRETRAINED_PATH:
            print(
                f"Loading PhysFormer weights from {config.MODEL.PHYSFORMER.PRETRAINED_PATH}"
            )
            phys_state = torch.load(
                config.MODEL.PHYSFORMER.PRETRAINED_PATH, map_location=self.device
            )
            # Handle potential DataParallel wrapping or different key names
            ...
            print(
                "Warning: No pretrained path provided for PhysFormer. This is critical for backend!"
            )
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                teacher_rPPG = (
                    bvp_label - torch.mean(bvp_label, axis=-1, keepdim=True)
                ...
                student_label = torch.zeros(compressed_vid.shape[0], num_classes).to(
                    self.device
                )
                student_label[:, 0] = 1.0
        ...
        if not self.config.TEST.USE_LAST_EPOCH:
            print(
                "best trained epoch: {}, min_val_loss: {}".format(
        ...
        if self.config.TRAIN.PLOT_LOSSES_AND_LR:
            self.plot_losses_and_lrs(
                mean_training_losses, mean_valid_losses, [], self.config
            )
    ...
    def save_model(self, index):
        ...
        model_path = os.path.join(
            self.model_dir, f"{self.model_file_name}_Joint_Epoch{index}.pth"
        )
