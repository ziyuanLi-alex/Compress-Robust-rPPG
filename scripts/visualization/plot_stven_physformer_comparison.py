...
def main():
    parser = argparse.ArgumentParser(description='Visualize STVEN+PhysFormer vs PhysFormer Degradation vs CRF')
    parser.add_argument('physformer_csv', type=str, help='Path to original PhysFormer results CSV')
    parser.add_argument('stven_csv', type=str, help='Path to STVEN+PhysFormer results CSV')
    ...
    df_stven = pd.read_csv(args.stven_csv)
    
    # Add Model column
    df_physformer['Model'] = 'PhysFormer'
    df_stven['Model'] = 'STVEN+PhysFormer'
    
    # Combine data
    combined_df = pd.concat([df_physformer, df_stven], ignore_index=True)
    
    # Set seaborn theme
    sns.set_theme(style="whitegrid", font="Times New Roman")
    sns.set_context("paper", font_scale=2.5)

    # Model colors - PhysFormer: blue, STVEN+PhysFormer: orange
    model_colors = {
        'PhysFormer': '#1f77b4',           # blue
        'STVEN+PhysFormer': '#ff7f0e'      # orange
    }
    ...
    for metric_idx, metric in enumerate(args.metrics):
        ...
        # Plot each model
        for model_idx, model in enumerate(['PhysFormer', 'STVEN+PhysFormer']):
        ...
        if metric == 'Pearson':
            ax_right.legend(loc='lower left', fontsize=18, frameon=False)
        else:
        ...
        output_path = output_dir / f'STVEN_PhysFormer_vs_PhysFormer_{metric}.png'
