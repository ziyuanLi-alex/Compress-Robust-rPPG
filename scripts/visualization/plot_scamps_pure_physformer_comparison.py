...
def main():
    parser = argparse.ArgumentParser(description='Visualize SCAMPS vs PURE PhysFormer Degradation vs CRF')
    parser.add_argument('pure_csv', type=str, help='Path to PURE PhysFormer results CSV')
    parser.add_argument('scamps_csv', type=str, help='Path to SCAMPS PhysFormer results CSV')
    ...
    # Load data
    df_pure = pd.read_csv(args.pure_csv)
    df_scamps = pd.read_csv(args.scamps_csv)
    
    # Add Dataset column
    df_pure['Dataset'] = 'PURE'
    df_scamps['Dataset'] = 'SCAMPS'
    
    # Combine data
    combined_df = pd.concat([df_pure, df_scamps], ignore_index=True)
    
    # Set seaborn theme
    sns.set_theme(style="whitegrid", font="Times New Roman")
    sns.set_context("paper", font_scale=2.5)

    # Dataset colors - PURE: blue, SCAMPS: green
    dataset_colors = {
        'PURE': '#1f77b4',      # blue
        'SCAMPS': '#2ca02c'     # green
    }
    ...
    for metric_idx, metric in enumerate(args.metrics):
        ...
        # Plot each dataset
        for dataset_idx, dataset in enumerate(['PURE', 'SCAMPS']):
        ...
        output_path = output_dir / f'SCAMPS_PURE_PhysFormer_{metric}.png'
