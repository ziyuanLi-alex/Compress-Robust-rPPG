...
def main():
    parser = argparse.ArgumentParser(description='Visualize PURE PhysFormer on UBFC-rPPG vs UBFC-Phys')
    parser.add_argument('ubfc_rppg_csv', type=str, help='Path to PURE PhysFormer UBFC-rPPG results CSV')
    parser.add_argument('ubfc_phys_csv', type=str, help='Path to PURE PhysFormer UBFC-Phys results CSV')
    ...
    # Load data
    df_ubfc_rppg = pd.read_csv(args.ubfc_rppg_csv)
    df_ubfc_phys = pd.read_csv(args.ubfc_phys_csv)
    
    # Add Dataset column
    df_ubfc_rppg['Dataset'] = 'UBFC-rPPG'
    df_ubfc_phys['Dataset'] = 'UBFC-Phys'
    
    # Combine data
    combined_df = pd.concat([df_ubfc_rppg, df_ubfc_phys], ignore_index=True)
    
    # Set seaborn theme
    sns.set_theme(style="whitegrid", font="Times New Roman")
    sns.set_context("paper", font_scale=2.5)

    # Dataset colors - UBFC-rPPG: blue, UBFC-Phys: purple
    dataset_colors = {
        'UBFC-rPPG': '#1f77b4',     # blue
        'UBFC-Phys': '#9467bd'      # purple
    }
    ...
    for metric_idx, metric in enumerate(args.metrics):
        ...
        # Plot each dataset
        for dataset_idx, dataset in enumerate(['UBFC-rPPG', 'UBFC-Phys']):
            dataset_data = combined_df[combined_df['Dataset'] == dataset]
            color = dataset_colors[dataset]
            
            # Main line plot for CRF >= 14 (right axis)
            data_main = dataset_data[dataset_data['CRF'] >= 14]
            ...
            data_crf0 = dataset_data[dataset_data['CRF'] == 0]
        ...
        output_path = output_dir / f'UBFC_rPPG_Phys_PhysFormer_{metric}.png'
