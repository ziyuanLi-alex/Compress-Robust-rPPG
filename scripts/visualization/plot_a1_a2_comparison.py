...
def main():
    parser = argparse.ArgumentParser(description='Visualize A1 vs A2 STVEN+PhysFormer Pretraining Comparison')
    parser.add_argument('a1_csv', type=str, help='Path to A1 (no pretrain) results CSV')
    parser.add_argument('a2_csv', type=str, help='Path to A2 (pretrained) results CSV')
    ...
    # Load data
    df_a1 = pd.read_csv(args.a1_csv)
    df_a2 = pd.read_csv(args.a2_csv)
    
    # Add Model column
    df_a1['Model'] = 'A1 (no pretrain)'
    df_a2['Model'] = 'A2 (pretrained)'
    
    # Combine data
    combined_df = pd.concat([df_a1, df_a2], ignore_index=True)
    
    # Set seaborn theme
    sns.set_theme(style="whitegrid", font="Times New Roman")
    sns.set_context("paper", font_scale=2.5)

    # Model colors - A1: blue, A2: green
    model_colors = {
        'A1 (no pretrain)': '#1f77b4',     # blue
        'A2 (pretrained)': '#2ca02c'       # green
    }
    
    # Metrics configuration with y-axis limits
    metrics_config = {
        'MAE': {'ylabel': 'MAE (bpm)', 'ylim': (0, 8)},
        'RMSE': {'ylabel': 'RMSE (bpm)', 'ylim': (0, 12)},
        'Pearson': {'ylabel': 'Pearson r', 'ylim': (0.7, 1.0)},
        'MAPE': {'ylabel': 'MAPE (%)', 'ylim': (0, 8)}
    }
    ...
    for metric_idx, metric in enumerate(args.metrics):
        ...
        # Plot each model
        for model_idx, model in enumerate(['A1 (no pretrain)', 'A2 (pretrained)']):
        ...
        output_path = output_dir / f'A1_A2_pretrain_comparison_{metric}.png'
