...
def main():
    parser = argparse.ArgumentParser(description='CRF Encoding Strategy Ablation: PhysFormer vs C1 vs C2')
    parser.add_argument('physformer_csv', type=str, help='Path to original PhysFormer results CSV')
    parser.add_argument('c1_csv', type=str, help='Path to C1 (limited encoding, 2 levels) results CSV')
    parser.add_argument('c2_csv', type=str, help='Path to C2 (sparse encoding, 4 levels) results CSV')
    ...
    df_c1 = pd.read_csv(args.c1_csv)
    df_c2 = pd.read_csv(args.c2_csv)
    
    # Add Model column
    df_physformer['Model'] = 'PhysFormer (no STVEN)'
    df_c1['Model'] = 'C1: limited (2 levels)'
    df_c2['Model'] = 'C2: sparse (4 levels)'
    
    # Combine data
    combined_df = pd.concat([df_physformer, df_c1, df_c2], ignore_index=True)
    ...
    # Model colors
    model_colors = {
        'PhysFormer (no STVEN)': '#d62728',    # red
        'C1: limited (2 levels)': '#9467bd',   # purple
        'C2: sparse (4 levels)': '#2ca02c'     # green
    }
    
    # Metrics configuration with y-axis limits
    metrics_config = {
        'MAE': {'ylabel': 'MAE (bpm)', 'ylim': (0, 30)},
        'RMSE': {'ylabel': 'RMSE (bpm)', 'ylim': (0, 40)},
        'Pearson': {'ylabel': 'Pearson r', 'ylim': (-0.2, 1.0)},
        'MAPE': {'ylabel': 'MAPE (%)', 'ylim': (0, 30)}
    }
    ...
    for metric_idx, metric in enumerate(args.metrics):
        ...
        # Plot each model
        for model_idx, model in enumerate(['PhysFormer (no STVEN)', 'C1: limited (2 levels)', 'C2: sparse (4 levels)']):
        ...
        # Add legend - position varies by metric
        if metric == 'Pearson':
            ax_right.legend(loc='lower left', fontsize=16, frameon=False)
        else:
            ax_right.legend(loc='upper left', fontsize=16, frameon=False)
        ...
        output_path = output_dir / f'CRF_encoding_ablation_{metric}.png'
